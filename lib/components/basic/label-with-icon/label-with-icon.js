...
import labelWithIconCssTemplate from "./label-with-icon.css";
import labelWithIconHtmlTemplate from "./label-with-icon.html";
...
class AppLabelWithIcon extends HTMLElement {
  constructor(
    ...
    basePath = "/components/core/label-with-icon",
    templateHtml = labelWithIconCssTemplate,
    templateStyleUrls = [labelWithIconHtmlTemplate],
  ) {
    super();
   ...
   */
  connectedCallback() {
    const _app_lwi_container = this.shadowRoot.querySelector(".label-with-icon");
  ...
  getElement() {
    ...
    const element = this.shadowRoot.querySelector("#label-with-icon");
    if (element) {
      element.remove();
    }

    let labelWithIcon = document.createElement("div");
    labelWithIcon.classList.add("label-with-icon");
    labelWithIcon.setAttribute("id", "label-with-icon");
    labelWithIcon.innerHTML = "";

    this.shadowRoot.appendChild(labelWithIcon);

    return labelWithIcon;
  }

  getTemplate() {
    return this.shadowRoot.querySelector("#label-with-icon-template");
  }
  ...
  render() {
    ...
    const labelWithIcon = this.getElement();

    if (labelWithIcon && "content" in document.createElement("template")) {
      const itemTemplate = this.getTemplateClone(this.getTemplate());

      // Apply data-attributes directly to the labelWithIcon element
      Object.keys(this.props).forEach((key) => {
        let value = this.props[key];

        if (!isEmptyValue(value)) {
          if (key === "color") {
            const color_changer = this.shadowRoot.getElementById("label-with-icon-input");
            color_changer.value = window.HSLToHex(getComputedStyle(document.documentElement, null).getPropertyValue("--color-primary"));
          } else if (key === "id") {
            labelWithIcon.setAttribute(`${key}`, value);
          }
        }
      });

      // Render children
      labelWithIcon.innerHTML = ""; // Clear existing content
      labelWithIcon.appendChild(itemTemplate);
    }
  }
}
