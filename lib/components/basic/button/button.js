...
export class AppButtonComponent extends HTMLElement {
  ...
  getElement() {
    ...
    const element = this.shadowRoot.querySelector(`#${this.props.id}` || "#button");
    if (element) element.remove();

    const button = document.createElement("button");
    button.classList.add("button");
    button.setAttribute("id", `${this.props.id}` || "button");
    button.innerHTML = "";
    ...
    if (button && "content" in document.createElement("template")) {
      ...
      if (this.props.type === "link" && !isEmptyValue(this.props.href)) {
        button.addEventListener("click", () => {
          const newWindow = window.open(this.props.href, this.props["href-target"] || "_blank", "noopener,noreferrer");
          if (newWindow) {
      ...
      if (this.props.type === "pwa-installer") {
        ...
        window.addEventListener("beforeinstallprompt", (event) => {
          ...
          button.setAttribute("style", "display: inline-flex !important");
        });
      }
