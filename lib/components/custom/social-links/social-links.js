...
export class AppSocialLinksComponent extends HTMLElement {
  constructor(
    props = {
      id: "social-links",
      color: "",
  ...
  showLinks() {
    ...
    if (this.hasAttribute("blog")) {
      const _app_link_blog = this.createLink("fas fa-rss", "Checkout my blog!", "Blog", `https://baijudodhia.blogspot.com/?utm_source=${utmSource}&utm_medium=website`);
  ...
  createLink(icon, iconTitle, title, link) {
    ...
    _app_link.setAttribute("shape", this.props.shape || "curved");
    _app_link.setAttribute("size", this.props.size || "s");
    _app_link.setAttribute("style", this.props.style || "");
    _app_link.setAttribute("appearance", this.props.appearance || "primary");
    return _app_link;
  }
  ...
  getElement() {
    ...
    const element = this.shadowRoot.querySelector(`#${this.props.id}` || "#social-links");
    if (element) {
      element.remove();
    }
    ...
    socialLinks.setAttribute("id", `${this.props.id}` || "social-links");
